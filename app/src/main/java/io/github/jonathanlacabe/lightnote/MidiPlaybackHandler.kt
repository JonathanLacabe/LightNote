...
class MidiPlaybackHandler(private val contentResolver: ContentResolver){
    ...
    //Constants for all the GM instrument names in midi, all 128 of them (the 47 drums are all under "Drum", so no extra array is required).
    ...
    //These variables store the elapsed time in milliseconds for the time counter text.
    private var elapsedMillis = 0L
    private var lastStartTime = 0L //Timestamp of last resume - if you pause at 0.99 ms, it should immediately go to 1 when you unpause, not forgetting the millisecond you were on.

    //Callback function - Update elapsed time on the UI
    ...
    private fun determineTrackType(inputStream: InputStream){
        try{
            ...
            var trackType = "Instrument" //Default to "Instrument" before doing anything
            ...
            for (track in sequence.tracks){
                for (i in 0 until track.size()){
                    ...
                    if (message != null && message.message?.isNotEmpty() == true){
                        ...
                        if (channel == 10){  //MIDI channel 10 is only for drums, nothing else. See parseInstrumentAndChannel for the other drum-error-fixing code.
                            trackType = "Rhythm"
                            break
                        }
                    }
            ...
            //Trigger the type callback with the newly determined track type:
    ...
    //Set the channel update callback:
    ...
    fun loadAndPlayMidiFile(uri: Uri, startTick: Long = 0L){
        stopPlayback()//Stop the playback (a pause):
        contentResolver.openInputStream(uri)?.use{ inputStream ->
            midiDriver.start()
            parseInstrumentAndChannel(contentResolver.openInputStream(uri)!!)
    ...
    //Stores the instrument for each channel:
            ...
            for (track in sequence.tracks){
                ...
                for (i in 0 until track.size()){
                    ...
                    if (message != null && message.message?.isNotEmpty() == true){
                        ...
                        //Set drum channel flag - This was causing issues for some incomprehensible reason, and this fixes it. Mysteries of programming.
                        if (channel == 10){
                            isDrumChannel = true
                            detectedChannel = 10
                        }

                        //program change for instrument detection -
    ...
    private fun playMidiData(inputStream: InputStream, startTick: Long){
        ...
        playbackThread = Thread{
            try{
                ...
                val tempoChanges = mutableListOf<Pair<Long, Long>>() //(tick, tempo in microseconds per quarternote)
                for (track in sequence.tracks){
                    for (i in 0 until track.size()){
                        val event = track.get(i)
                ...
                for ((tick, message, channel) in allEvents){
                    if (!isPlaying) break //I used a very cool single line conditional here, take notes programmers
                    ...
                    //Send correct message based on the channel of the file:
    ...
    fun resumePlayback(uri: Uri){
        if (!isPlaying) {
            ...
            contentResolver.openInputStream(uri)?.let{ inputStream ->
                //Load and start playing the MIDI file from the resumeTick (tick that the user pressed pause or had something trigger a pause)
                loadAndPlayMidiFile(uri, resumeTick)

                /*After playback has started, restore the correct instruments.
                  Also ensures that Program Change messages are sent after playback resumes.
                */
                Handler(Looper.getMainLooper()).postDelayed({
                    for ((channel, instrument) in channelInstruments){
                        //Send Program Change message to set the correct instrument (:D)
    ...
    //Helper function to retrieve total elapsed time in milliseconds (to prevent incorrect information in the time textbox):
    ...
    //Stops playback and reset every variable. Used when the user opens another midi after having one already open.
    fun resetPlaybackState(){
        stopPlayback()

        //Reset every variables to initial values
        isPlaying = false
        isPaused = false
        currentTick = 0L
        resumeTick = 0L
